...
import { Play, RefreshCcw, ShieldCheck, Zap } from "lucide-react";
...
export function TradeCopyDemo() {
  ...
  return (
    <section
      id="trade-copy-demo"
      className="mx-auto max-w-7xl scroll-mt-24 px-5 py-24 sm:px-6 lg:px-8"
    >
      <div className="mx-auto max-w-4xl text-center">
        <p className="mb-4 text-xs font-bold uppercase tracking-[0.26em] text-electric">
          Live Trade Copy Visualization
        </p>

        <h2 className="text-balance text-4xl font-black text-white sm:text-5xl">
          Watch a master trade{" "}
          <span className="text-gradient">sync across accounts</span>
        </h2>

        <p className="mx-auto mt-5 max-w-3xl text-lg leading-8 text-steel">
          A premium visual demo showing how one master trade can be copied to
          connected slave accounts with execution status, delay tracking and
          synchronized trade details.
        </p>
      </div>

      <div
        className={`demo-shell ${active ? "demo-active" : ""} mt-14 rounded-3xl border border-line bg-panel/70 p-5 shadow-glow sm:p-8`}
      >
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-line bg-white/[0.03] p-4">
          <div className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl bg-electric/10 text-electric ring-1 ring-electric/20">
              <Zap className="h-5 w-5" />
            </span>
            <div>
              <p className="text-sm font-bold text-white">
                Professional Master → Slave Workflow
              </p>
              <p className="text-xs text-steel">
                Visual demo only. Real execution depends on broker, server,
                connection and account conditions.
              </p>
            </div>
          </div>
        ...
        <div className="grid items-center gap-8 lg:grid-cols-[0.95fr_0.28fr_1.15fr]">
          <div className="premium-card premium-glow rounded-xl border border-electric/70 bg-ink/75 p-5">
            <div className="flex items-start justify-between">
              ...
              <span
                className={`h-3 w-3 rounded-full ${
                  state === "copying" ? "animate-pulse bg-electric" : "bg-emerald"
                }`}
              />
            </div>
          ...
          <div className="relative hidden h-72 lg:block" aria-hidden="true">
            <svg
              key={run}
              viewBox="0 0 170 290"
              className="h-full w-full overflow-visible text-cyan/70"
            >
              <path
                className="demo-path"
                d="M0 145 C80 145 70 35 170 35"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              />
              <path
                className="demo-path"
                d="M0 145 C70 145 90 145 170 145"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              />
              <path
                className="demo-path"
                d="M0 145 C80 145 70 255 170 255"
                fill="none"
                stroke="currentColor"
                strokeWidth="2"
              />
          ...
          <div className="grid gap-4">
            {slaveAccounts.map((account, index) => {
              ...
              const label =
                state === "idle" ? "Idle" : copied ? "Copied" : "Copying";

              return (
                <div
                  key={account.id}
                  className={`premium-card rounded-xl border p-5 transition duration-500 ${
                    active
                      ? "border-emerald/40 bg-emerald/[0.045]"
                      : "border-line bg-ink/70"
                  }`}
                  style={{
                    transitionDelay: state === "copying" ? account.delay : "0ms",
                  }}
                >
                  ...
                  <div
                    className={`mt-4 grid grid-cols-3 gap-3 text-xs transition ${
                      active ? "opacity-100" : "opacity-45"
                    }`}
                  >
                    <span className="rounded border border-line bg-ink/50 px-3 py-2 text-steel">
                      XAUUSD
                    </span>
                    <span className="rounded border border-line bg-ink/50 px-3 py-2 font-bold text-emerald">
                      BUY
                    </span>
                    <span className="rounded border border-line bg-ink/50 px-3 py-2 text-right font-mono text-white">
                      1.00
                    </span>
                  </div>
                </div>
        ...
        <div className="mt-8 flex flex-col items-center justify-between gap-3 border-t border-white/[0.08] pt-5 text-sm text-steel sm:flex-row">
          ...
          <span className="font-mono text-xs uppercase tracking-[0.18em] text-steel">
            Master to Slave 1, Slave 2, Slave 3
          </span>
        </div>
      </div>
    </section>
